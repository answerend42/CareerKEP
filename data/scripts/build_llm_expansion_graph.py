"""Compile the expanded KG raw graph bundle from preserved seed edges and LLM judgments.

The output graph is review-oriented rather than runtime-ready:
- seed edges are preserved as the current formal graph skeleton
- LLM edges keep review status and confidence
- new LLM edges leave weight as null until manual assignment is completed

Outputs:
- data/entity_expansion/llm_expanded_graph.raw.json
- frontend/public/kg-expanded-overview-data.json
"""
...
DEFAULT_LLM_RESULTS = (
    REPO_ROOT / "data" / "entity_expansion" / "llm_edge_candidates.jsonl"
)
DEFAULT_OUTPUT = REPO_ROOT / "data" / "entity_expansion" / "llm_expanded_graph.raw.json"
DEFAULT_OVERVIEW_OUTPUT = (
    REPO_ROOT / "frontend" / "public" / "kg-expanded-overview-data.json"
)
...
def main() -> None:
    ...
    llm_rows = load_jsonl(Path(args.llm_results))
    ...
    llm_kept_rows = [row for row in llm_rows if row.get("status") != "rejected_none"]
    rejected_none_count = sum(
        1 for row in llm_rows if row.get("status") == "rejected_none"
    )

    graph_bundle = {
        "schema": "career_kep.llm_expanded_graph.raw.v1",
        ...
        "audits": {
            "llm_result_row_count": len(llm_rows),
