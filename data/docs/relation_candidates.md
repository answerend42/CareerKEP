...
- `pair_source_id` / `pair_target_id`：证据中按出现顺序提取出的实体对。
- `source_id` / `target_id`：最终选中的关系方向。
- `relation_type`：最终选中的关系类型。
- `selected_direction`：`forward` 或 `reverse`。
- `selected_candidate_rank`：最终命中的候选在对应方向列表中的排序名次，越小越优。
- `selected_candidate`：最终命中的候选快照，保留 `keyword_count`、`base_weight`、`selection_score` 和 `candidate_rank`，便于复核。
- `selection_reason`：简短的选择依据，说明本次为什么选中这条候选。
...
- 当 `relation_instances.json` 和 `relation_candidates.json` 不一致时，优先检查候选轨迹里的 `forward_candidates`、`reverse_candidates` 和 `selected_candidate`。
...
- `selected_candidate` 是最值得复核的字段，因为它同时保留了分数、排序和方向信息。
