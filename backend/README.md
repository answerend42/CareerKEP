...
`GET /api/meta` 返回图谱节点数、边数、分层统计、关系统计、聚合器统计、别名统计、基础校验状态、所有 `role` 节点列表，以及可直接用于前端搜索下拉的 `role_options` 和 `role_search_index`。`graph.validation.status` 在没有告警时为 `ok`，一旦存在本地 warning 会变成 `warn`。`graph.validation.warnings` 会把当前图谱和别名词典的本地告警一起带出来，前端或启动脚本可以直接据此判断要不要提示用户。
`graph.connectivity` 会额外暴露根节点、叶子节点和孤立节点的数量及节点 ID 列表，方便前端和启动脚本快速判断图谱是否存在断层或覆盖不足的问题。
...
`input_trace` 里还会返回 `structured_evidence_warnings`，专门记录哪些结构化证据因为空 ID、非法分值或布尔值被跳过，方便前端和脚本排查脏数据来源。
