...
from data_engine.core.paths import DEFAULT_VIZ_OUTPUT, SEED_EDGES, SEED_NODES
...
<header><h1>CareerKEP 知识图谱</h1>
<p>{len(nodes)} 节点 · {len(edges)} 边</p></header>""",
    ...
    for layer in LAYERS:
        parts.append(
            ...
            f'fill="#8a93b1" font-size="14">{LAYER_LABEL[layer]}（{by_layer_count.get(layer, 0)}）</text>'
        )
    ...
    for edge in edges:
        ...
        parts.append(
            f'<path d="M{sx},{sy} C{cx},{sy} {cx},{ty} {tx - 6},{ty}" '
            f'stroke="{color}" stroke-width="1.4" fill="none" opacity="{opacity:.2f}" '
            f'marker-end="url(#arrow-{edge["relation"]})"/>'
        )
...
def render(output_path: Path | None = None) -> Path:
    ...
    target = output_path or DEFAULT_VIZ_OUTPUT
