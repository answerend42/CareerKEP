...
对于 JSON 快照，采集器还会继续检查顶层记录里的 `metadata` 和 `extra`，避免标题、正文或来源字段被埋在嵌套对象后面时静默漏采。
...
- 抽取阶段会优先保留更可信的别名来源，来源优先级为 `explicit > label > id > id_words > generated`，避免生成别名覆盖显式词典别名。
- 消歧阶段会先按实体 ID 去重候选，再做排序和打分，避免同一个实体因为多个别名来源重复占位，污染 `candidate_count` 和 `score_gap`。
